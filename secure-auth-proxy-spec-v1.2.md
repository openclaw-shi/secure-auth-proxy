...
プロキシはAPIキーの**窃取**を防ぐが、**権限の行使**は防げない。エージェントはプロキシを通じて認証済みリクエストを送れるため、キーに紐づく権限（高額APIコール等）を悪用される可能性がある。MVPではレートリミットで緩和し、API提供元のダッシュボードで課金上限を設定する運用とする。
...
- **ユーザー**: 2つのLinuxユーザーを使用
  - `proxy-admin`: 認証プロキシを実行。暗号化キーファイルの所有者
  - `agent`: エージェントを実行
...
| keys.encの所有者 | proxy-admin:proxy-admin | `stat /opt/auth-proxy/keys.enc` |
| keys.encのパーミッション | 600 | 同上 |
...
```
[初回セットアップ]
パスワード → scrypt (salt=ランダム) → 導出鍵 → AES-256-GCM暗号化 → keys.enc

[プロキシ起動時]
keys.enc + パスワード入力 → scrypt → 導出鍵 → 復号 → メモリ上の変数に保持
※ 復号結果はファイルに書き戻さない
※ 環境変数にも入れない
```

Node.jsの `crypto.scrypt` + ランダムsalt + AES-256-GCM で実装する。

### 4.2 復号済みキーの保護

復号済みAPIキーは `proxy-admin` プロセスのメモリ上にのみ存在する。
...
| ファイルシステム経由 | 不可 | keys.encは暗号化済み + 600 |
...
2. 非対応 → TCPフォールバック + iptables UID制限
...
# → パスワード入力 → 各サービスのAPIキーを入力 → keys.enc に保存
chmod 600 keys.enc config.yaml
exit
```
...
# 2. プロキシ起動
sudo -u proxy-admin node /opt/auth-proxy/server.js
# → パスワード入力 → "Proxy listening on /run/auth-proxy/proxy.sock"
...
`crypto.scrypt` から `Argon2id` へ移行、または `age` / `libsodium` 等の既存フォーマットの採用。

### 9.5 秘密保持部分の言語分離

キーの復号・保持・ヘッダ注入をRust/Go/Cの小さなデーモンに切り出す。メモリのzeroize、`mlock()` 等が可能になる。
...
`systemd-ask-password` やLinux Keyring / TPM経由での秘密投入を検討。
